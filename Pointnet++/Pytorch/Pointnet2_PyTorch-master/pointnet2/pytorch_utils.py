...
import os
from .utility_cm import cm_stat, save_epoch, save_results, save_epoch_val
...
class Trainer(object):
    ...
    def _train_it(self, it, batch):
        ...
        # _, loss, eval_res = self.model_fn(self.model, batch)  # TODO roessl
        _, loss, eval_res, cm_list = self.model_fn(self.model, batch) # cm: confusionmatrix_data

        loss.backward()
        self.optimizer.step()

        return eval_res  # TODO roessl
    ...
    def eval_epoch(self, d_loader):
        ...
        count = 1.0
        cm_dict = {} # roessl
        # roessl append cm data
        # for key in cm_list.keys():
        #     cm[key].append(cm_list[key])
        cm_dict= {}
        ...
        #         total=len(d_loader),
        #         leave=False,
        #         desc='val'):  # TODO roessl: comment-pgb

        for i, data in enumerate(d_loader, 0):
            self.optimizer.zero_grad()

            # _, loss, eval_res = self.model_fn(self.model, data, eval=True) #TODO roessl
            _, loss, eval_res, cm_data = self.model_fn(self.model, data, eval=True)  # roessl cm: confusionmatrix data
            if bool(cm_dict):
                for key in cm_data.keys():
                    cm_dict[key].extend(cm_data[key])
            else:
                for key in cm_data.keys():
                    cm_dict[key] = cm_data[key]  # roessl
        ...
        # return total_loss / count, eval_dict # TODO roessl
        return total_loss / count, eval_dict, cm_dict
    ...
    def train(self,
              ...
              test_loader=None,
              best_loss=0.0,
              folder=False,  # Roessl added folder to save results
              save_validation=False # boolean in order to save the test result; False to save time or if shuffle test data is used
              ):
        r"""
        ...
        storage_results = np.zeros([n_epochs, 3])
        ...
        with tqdm.trange(start_epoch, n_epochs + 1, desc='epochs') as tbar, \
            ...
            for epoch in tbar:
                print("\nEpoch: " + str(epoch))  # TODO implement lambda
                train_store = [0, 0]
                # cm_dict={'actual': list(), 'prediction': list()} # roessl
                for batch in train_loader:
                    res = self._train_it(it, batch)
                    # roessl
                    len_batch = batch[1].shape[0]
                    train_store[0] += res['acc'] * len_batch
                    train_store[1] += len_batch
                    it += 1
                    ...
                    if (it % eval_frequency) == 0:
                        ...
                        if test_loader is not None:
                            val_loss, res, cm_dict = self.eval_epoch(test_loader)
                            if save_validation:
                                accuracy = sum([1 if x == y else 0 for x,y in
                                                zip(cm_dict['actual'],cm_dict['prediction'])]) / len(cm_dict['actual'])
                                save_epoch_val(actual=cm_dict['actual'], prediction=cm_dict['prediction'],
                                               accuracy=accuracy, epoch=epoch, path=folder)
                            # roessl append cm data
                    ...
                    self.viz.flush()
                # # roessl: save results of the current epoch
                epoch_str = ["Epoch: " + str(epoch)]
                # print("Scikit cohen's kappa: %.4f" % cohen_kappa_score(cm_dict["actual"], cm_dict["prediction"]))
                ...
                storage_results[epoch - 1, 0:2] = cm_stat(confusion_mat)
                storage_results[epoch - 1, 2] = train_store[0] / train_store[1]
                epoch_str.append(confusion_mat)
                epoch_str.append("OA: " + str(int(storage_results[epoch - 1, 0] * 100)) +
                                 "%\nCohen's kappa: " + str(int(storage_results[epoch-1, 1] * 100) / 100))
                epoch_str.append(classification_report(y_true=cm_dict['actual'], y_pred=cm_dict['prediction'], labels=cm_labels))
                if folder:
                    save_epoch(epoch_str, path=folder)
                    sleep(1)  # sleeps for 1000 ms to update the console => 1000epochs = 1000sec ~15min
                    if epoch == n_epochs :
                        save_results(storage_results, path=folder)
                        last_rounds = int(n_epochs * 0.3)
                        print("last epoch.\nMean Accuracy last 30 percent of epochs: %s\nMean cohens_kappa last 30 percent of epochs: "
                              "%s" % (str(np.mean(storage_results[-last_rounds :, 0])),
                                      str(np.mean(storage_results[-last_rounds :, 1])))) # roessl_end_comment; line 761
        return best_loss
